	  ...
	  if (get_FEM_vtu_data(input_folder, num_parts, solution_step, "Solution", false, 0)){ 
		...
		if (!pointlist_init){
			for (int nd = 0; nd < nodelist.size(); nd++){
				//See if the node is located in the thin disk specified in the "thermocouple_1" node location test
				if ( thermocouple_1(nodelist[nd].x, nodelist[nd].y, nodelist[nd].z)){
		...
		std::cout << "Thermocouple temperature computed \n\n";
	  }
	  std::getline(input_pvd,line_pvd);
	  nodelist.clear();
	  elementlist.clear();
...
void create_grid_csv_temp_hist_file_consol(std::string input_filepath_pvd, std::string input_filename_base, int num_parts, std::string output_filename, 
	...
	outfile_temp_hist << "Regular Grid Temperature History for Simulation \"" << input_filename_base << "\"\n"; 
	...
	*/
	...
	while (line_pvd.find("</Collection>") > 5000){ //Iterate until the end of the simulation timesteps 
		...
		if (get_FEM_vtu_data(input_folder, num_parts, solution_step, "Solution", false, 0)){
			...
			outfile_temp_hist << setprecision(7) << timestep << ",";
...
void create_grid_csv_temp_hist_file(std::string input_filepath_pvd, std::string input_filename_base, int num_parts, std::string output_filename, 
	...
	outfile_temp_hist << "Regular Grid Temperature History for Simulation \"" << input_filename_base << "\" " << std::endl; 
	//This can be uncommented for debug and to see the order in which nodes are stacked into the output file
	/*
    outfile_temp_hist << "Regular Grid Point Number, X Coord, Y Coord, Z Coord" << "\n";
	for (int pt_ct = 0; pt_ct < pointlist.size(); pt_ct++){
	...
	*/
	...
	outfile_temp_hist << "Time (us), Temperature at Regular Grid Points \n"; 
	std::ostringstream ss_pvd;
	...
	while (line_pvd.find("</Collection>") > 5000){ //Iterate until the end of the simulation timesteps 
		...
		if (get_FEM_vtu_data(input_folder, num_parts, solution_step, "Solution", false, 0)){
			//Create and init. array to keep track and make sure all regular grid points are contained in the FEM unstructured mesh
		  	std::cout<<"i'm here 1\n";
			std::cout<< "pointlist size is " <<pointlist.size()<<"\n";
